...
class TestFetchDartDisclosures:
    ...
    def test_parses_disclosure_list(self):
        ...
        with patch.object(module, "_fetch_json", side_effect=self._mock_fetch([company_resp, list_resp])):
...
class TestIngestAllNews:
    def test_routes_us_ticker_to_av(self, monkeypatch):
        ...
        av_mock = MagicMock(return_value=[{"title": "t", "url": "u", "published_at": "",
                                           "source": "s", "summary": "", "sentiment_score": 0.0}])
        with patch.object(module, "_fetch_av_news", av_mock), \
    ...
    def test_routes_kr_ticker_to_dart(self, monkeypatch):
        ...
        with patch.object(module, "_fetch_dart_disclosures", dart_mock), \
             patch.object(module, "_cache_news"), \
             patch.object(module, "_save_to_db"):
            result = module.ingest_all_news(["005930"])
    ...
    def test_records_error_on_per_ticker_failure(self, monkeypatch):
        ...
        with patch.object(module, "_fetch_av_news", side_effect=Exception("rate limit")), \
             patch("time.sleep"):
            result = module.ingest_all_news(["AAPL"])
