...
# 캐시 TTL (초)
PRICE_TTL_MARKET_HOURS = 900     # 장 중 15분
PRICE_TTL_AFTER_CLOSE = 86_400   # 장 마감 후 24시간

# 장 중 판단 기준 (UTC, weekday 0=월 ~ 6=일)
...
class TechnicalRequest(BaseModel):
    ...
    def validate_params(self) -> TechnicalRequest:
        """ticker 형식, market, period 값을 검증합니다."""
        if self.market == "KR":
...
def _get_price_ttl(market: str) -> int:
    """장 중/마감 여부에 따른 캐시 TTL(초)을 반환합니다."""
...
def _date_range(period: str) -> tuple[str, str]:
    """period 문자열을 (from_date, to_date) 쌍으로 변환합니다."""
