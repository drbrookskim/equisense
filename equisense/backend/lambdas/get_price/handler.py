...
from core.response import ok, error as api_error
...
def lambda_handler(event: dict, context: Any) -> dict:
    ...
    ticker: str = (
        (event.get("pathParameters") or {}).get("ticker", "").upper()
    )
    ...
    if not TICKER_RE.match(ticker):
        return {
            "statusCode": 400,
            "headers": _CORS,
            "body": json.dumps({"error": {"code": "INVALID_TICKER", "message": "Invalid ticker format"}}),
        }
    ...
    try:
        ...
        return {
            "statusCode": 503,
            "headers": _CORS,
            "body": json.dumps({"error": {"code": "CACHE_UNAVAILABLE", "message": "Cache unavailable"}}),
        }
